...
class ESP(object):
  ...
  def __del__(self):
    if hasattr(self, 'ser'): # # IN DEL, ser is NOT recognized !!
      self.ser.close()
      
  def closeESP(self):
    # # print(self.ser)
    if hasattr(self, 'ser'):
      print('In ESP, delete port')
      self.ser.flushInput()
  ...
  def write(self, string, axis=None):
    ...
    self.ser.write(((str(axis) if axis is not None else "") + string + "\r").encode())
...
class Axis(object):
  ...
  def home_search(self, mode=None, blocking=False):
    """Search for home.
  ...
  # # @property
  def moving(self):
    """Return True is motion is in progress.
    
    :rtype: bool
    """
    return False if self.query('MD?') == b'1' else True
  ...
  #catch_error
  def move_to(self, pos, blocking=False): 
    """Go to absolute position.
    
    :param pos: the final position (in mm)
    :type pos: float
    :param blocking: whether to block until the motion is finished
    :type blocking: bool
    """
  ...
  #catch_error
  def move_by(self, pos, blocking=False):
