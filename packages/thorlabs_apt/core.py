...
class Motor(object):
    ...
    def set_dc_profile_mode_parameters(self, profile_mode, jerk):
        ...
        jerk : float
        """
        err_code = _lib.MOT_SetDCProfileModeParams(self._serial_number, profile_mode, jerk)
        if (err_code != 0):
...
def _load_library():
    ...
    # version used before 04/08/2017
    '''
    path0 = os.getcwd()
    ...
    lib = None
    try:
        filename = ctypes.util.find_library("APT.dll")
        print('Library found in #1 by util')
    except:
        filename = None
        
    if (filename is not None):
        lib = ctypes.windll.LoadLibrary(filename)
        print('Library loaded in #1 by windll')
    else:
        try:
            filename = "%s/APT.dll" % path0
            print(path0)
        except:
            filename = "APT.dll"
            print('no path0')
            
        lib = ctypes.windll.LoadLibrary(filename) # windll leads to MOT_LLSetEncoderCount not found but that's all
        # WARNING : WORKED (could set home the motors and move them) 2016.7.8 !!
        # # lib = ctypes.CDLL(filename) # CDLL leads to errors like in lib.EnableEventDlg(False)
        print('Library loaded in #2 by windll')
        # in iMic or here, loaded by CDLL, but error 'MOT_LLSetEncoderCount' not found
        if (lib is None):
            filename = "%s/APT.dll" % os.path.dirname(sys.argv[0])
            lib = ctypes.windll.LoadLibrary(lib)
            print('Library loaded in #3 by windll')
            if (lib is None):
    ...
    # new version
    path0 = os.getcwd()
    # version used now (faster)
    print(_lib is None)
    if _lib is None:
        ff = 'APT'
    else:
        ff = 'APT2'
    filename = "%s/%s.dll" % (path0, ff)
    # libHandle = ctypes.windll.kernel32.LoadLibrary(filename) 
    # # lib = ctypes.cdll.LoadLibrary(filename) 
    lib = ctypes.windll.LoadLibrary(filename) 
    # # lib = ctypes.WinDLL(None, handle=libHandle)
    print('Library loaded by windll')
    _APTAPI.set_ctypes_argtypes(lib)
    if (lib.APTInit() != 0):
        raise Exception("Thorlabs APT Initialization failed.")
    if (lib.EnableEventDlg(False) != 0):
